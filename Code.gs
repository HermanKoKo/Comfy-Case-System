...
function getMaintenanceHistory(clientId) {
  try {
    ...
    const results = data.slice(1)
      ...
      .map(row => {
        // ★ 修改：手動映射欄位，確保 rr (J欄) 與 remark (K欄) 正確回傳
        // 假設欄位順序：A=ID, B=ClientID, C=Date, D=Staff, E=Item, F=BP, G=SpO2, H=HR, I=Temp, J=RR, K=Remark
        return {
          id: row[0],
